...
function applyCrop() { canvasAreaRef.value?.applyCrop() }
function cancelCrop() { currentTool.value = 'move'; canvasAreaRef.value?.cancelTool() }
function applyBgRemove() { canvasAreaRef.value?.applyBgRemove() }
function applyResize() { canvasAreaRef.value?.applyResize(resizeWidth.value, resizeHeight.value) }
function exportImage(opts) { canvasAreaRef.value?.exportImage(opts); showExport.value = false }
...
watch([colorBrightness, colorContrast, colorTemperature, colorSaturation], () => {
  if (currentTool.value === 'color') {
    canvasAreaRef.value?.applyColorPreview(
      colorBrightness.value, colorContrast.value,
      colorTemperature.value, colorSaturation.value
    )
  }
})
function setZoom(v) { zoom.value = Math.min(8, Math.max(0.1, v)) }
...
provide('editor', reactive({
  currentTool, hasImage, isProcessing, zoom,
  canUndo, canRedo,
  cropAspect, spotSize, spotStrength,
  resizeWidth, resizeHeight, resizeConstrain,
  setTool, performUndo, performRedo, loadFile,
  applyCrop, cancelCrop, applyBgRemove, applyResize,
  colorBrightness, colorContrast, colorTemperature, colorSaturation,
  applyColor, cancelColor, resetColorSliders,
  setZoom,
  openExport: () => { showExport.value = true },
}))
</script>
...
<template>
  <div class="app" @keydown="onKeyDown" tabindex="-1">
    ...
    <Transition name="fade">
      <ExportModal v-if="showExport" @close="showExport = false" @export="exportImage" />
    </Transition>
  </div>
</template>
