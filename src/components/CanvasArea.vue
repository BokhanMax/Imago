...
const displayWidth = computed(() => (mainCanvas.value?.width ?? 0) * editor.zoom)
const displayHeight = computed(() => (mainCanvas.value?.height ?? 0) * editor.zoom)
...
function applyColorPreview(brightness, contrast, temperature, saturation) {
  ...
  ctx.putImageData(new ImageData(copy, colorOrigData.width, colorOrigData.height), 0, 0)
}
...
function getSnapshot() {
  return new Promise(resolve => {
    const c = mainCanvas.value
    c.toBlob(blob => resolve({ blob, width: c.width, height: c.height }), 'image/png')
  })
}
...
function onCanvasMouseMove(e) {
  ...
  if (lastHealX !== null) {
    const steps = Math.max(1, Math.ceil(Math.hypot(x - lastHealX, y - lastHealY) / (editor.spotSize.value * 0.4)))
...
function onWheel(e) {
  if (!e.ctrlKey) return
  e.preventDefault()
  const steps = [0.1, 0.15, 0.25, 0.33, 0.5, 0.67, 0.75, 1, 1.25, 1.5, 2, 3, 4, 5, 6, 8]
...
defineExpose({ loadFile, fitToWindow, activateTool, cancelTool, applyCrop, applyBgRemove, applyResize, applyColorPreview, commitColor, getSnapshot, restoreSnapshot, exportImage })
</script>

<template>
  <div class="canvas-area">
    <!-- Drop zone -->
    <div v-if="!editor.hasImage" class="drop-zone"
      ...
      <div class="drop-inner">
        <div class="drop-icon">
          <svg width="48" height="48" viewBox="0 0 48 48" fill="none">
            ...
            <circle cx="17" cy="19" r="3" stroke="currentColor" stroke-width="1.8"/>
            <path d="M6 32l9-9 6 6 6-8 15 10" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>
          </svg>
        </div>
        <p class="drop-title">Перетащите изображение сюда</p>
        <p class="drop-subtitle">или нажмите кнопку <strong>Открыть</strong> на панели выше</p>
        <p class="drop-formats">PNG · JPG · WEBP · GIF</p>
      </div>
    </div>
...
.drop-subtitle { font-size: 12.5px; }
.drop-formats { font-size: 11px; color: var(--text-tertiary); margin-top: 4px; }
